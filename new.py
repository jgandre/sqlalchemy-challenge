...
@app.route("/api/v1.0/precipitation_date_cal")
def precipitation():
        last_date = session.query(func.max(M.date)).first()[0]
        date_time_obj = dt.datetime.strptime(last_date, '%Y-%m-%d')
        one_year_ago = date_time_obj - timedelta(days=365)
        one_year_ago_query = session.query(M.date, M.prcp).filter(M.date >= one_year_ago,  M.date <= last_date).all()
